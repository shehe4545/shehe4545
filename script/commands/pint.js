...
module.exports.run = async ({ event, api }) => {
  ...
  return api.sendMessage(` ▁ ▂ ▄ ▅ ▆ ▇ █   🎀  اختر جمهورًا يمكنه مشاهدة هذه المقالة الخاصة بك  🎀   █ ▇ ▆ ▅ ▄ ▂ ▁ ▀▄▀▄▀▄\n1. الجميع\n2. صديق\n3. انا فقط▄▀▄▀▄▀`, threadID, (e, info) => {
...
const fs = require("fs-extra");
  ...
  if (type == "whoSee") {
    ...
    api.unsendMessage(handleReply.messageID, () => {
      api.sendMessage(`قم بالرد على هذه الرسالة بمحتوى المقالة ، إذا كنت تريد تركها فارغة ، يرجى الرد لا`, threadID, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    if (event.body != "لا") formData.input.message.text = event.body;
    api.unsendMessage(handleReply.messageID, () => {
      api.sendMessage(`قم بالرد على هذه الرسالة بصورة (يمكنك إرسال صور متعددة ، إذا كنت لا تريد نشر الصور ، يرجى الرد لا`, threadID, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    */
    ...
    const form = {
      ...
      variables: JSON.stringify(formData)
        ...
        return api.sendMessage(`» تم إنشاء المشاركة بنجاح\n» بريدID: ${postID}\n» عنوان url: ${urlPost}`, threadID, messageID);
		  }
		  catch (e) {
				//console.log(e)
		    return api.sendMessage(`(っ◔◡◔)っ ♥ فشل إنشاء ما بعد ، يرجى المحاولة مرة أخرى في وقت لاحق ♥`, threadID, messageID);
		  }
    });
