...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("ضع تاك لشخص", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "مبروك تم قبض عليك  " + tag + '\n اتمنى لك سعاده في سجن 😇😇',
            mentions: [{
          tag: tag,
          id: mention
        }],
