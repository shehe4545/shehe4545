module.exports.config = {
  ...
  hasPermssion: 0,
  credits: "benzo",
  description: "صور اياكا من لعبة كنشن",
...
var min = Math.floor(Math.random() * 2);
  ...
  var money = data.money
      if(money < 100) api.sendMessage("تحتاج 100 دولار حتى تشوف الصور",event.threadID,event.messageID)
          else {
   Currencies.setData(event.senderID, options = {money: money - 100})
   var callback = () => api.sendMessage({body:`صور اياكا من لعبة كنشن\nعدد الصور: ${link.length}\n-تم خصم 100 دولار  !`,attachment: fs.createReadStream(__dirname + "/cache/1.jpg")}, event.threadID, () => fs.unlinkSync(__dirname + "/cache/1.jpg"), event.messageID); 
