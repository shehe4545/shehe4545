...
module.exports.run = async ({ event, api, args, Currencies, Users }) => {
	...
	if (args.length == 0) {
		...
		if (rank == 0) return api.sendMessage("يرجى الانتضار قليلا .", event.threadID, event.messageID);
	...
	if (mention.length == 1) {
		...
		if (rank == 0) return api.sendMessage("حاول بعد 5 دقائق .", event.threadID, event.messageID);
	...
	if (mention.length > 1) {
		for (const userID of mention) {
			...
			if (rank == 0) return api.sendMessage("حاول لاحقا .", event.threadID, event.messageID);
