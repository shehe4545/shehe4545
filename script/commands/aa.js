...
module.exports.run = async function ({ event, api, args, Currencies }) { 
  ...
  if (!two) return api.sendMessage("Please tag 1 person", threadID, messageID);
        ...
        return makeImage({ one, two }).then(path => api.sendMessage({ body: `[❤️] مستوى المودة بينك وبين هذا الشخص: ${hc} %\n[❤️] أنتما الاثنان تنعمان بـ ههههه: ${((hc)*rd)} $\n[❤️] أتمنى لك السعادة 🍀`, attachment: fs.createReadStream(path)}, threadID, () => fs.unlinkSync(path), messageID));
  }
}