...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
  ...
  var tl = ["تحتاج شيئا عزيزي ؟ " , "اتركني لست في مزاج جيد " , "هل تريد ان تعترف لي بشيء 🤭" , "اشتقت لك 🥰" , "انا في خدمتك" , "لن اجيبك لان عبدو يغار 🤭❤️" , "هففف ماذا مجددا " , "سام سالي سالي ارحموني🤬" , "سالي الحلوة فخدمتك" , " انا تحت امرك يا سيد" , "لن تتعبو من مناداتي ؟ 😠👊🏻"];  var rand = tl[Math.floor(Math.random() * tl.length)]
   ...
   if ((event.body.toLowerCase() == "احمد ") || (event.body.toLowerCase() == "كايت") || (event.body.toLowerCase() == "kite") || (event.body.toLowerCase() == "majd")) {
  ...
  if ((event.body.toLowerCase() == "ادمن") || (event.body.toLowerCase() == "صند ادمن")) {
   ...
   if ((event.body.toLowerCase() == "مح") || (event.body.toLowerCase() == "بوسيني")) {
     return api.sendMessage("️مح على احمد وحدو", threadID);
   };
   ...
   if ((event.body.toLowerCase() == "ما اسمك") || (event.body.toLowerCase() == "اسمك") || (event.body.toLowerCase() == "ما اسمها")) {
     return api.sendMessage("️سام تشرفت بمعرفتك.", threadID);
   };
...
module.exports.run = function({ api, event, client, __GLOBAL }) { }const fs = global.nodemodule["fs-extra"];
module.exports.config = {
  name: "goibot",
  version: "1.0.1",
  hasPermssion: 0,
...
module.exports.handleEvent = async function({ api, event, args, Threads, Users }) {
  ...
  if ((event.body.toLowerCase() == "ادمن") || (event.body.toLowerCase() == "صند ادمن")) {
   ...
   if ((event.body.toLowerCase() == "هل تحبينني ؟") || (event.body.toLowerCase() == "هل سالي تحبني ؟")) {
     return api.sendMessage("اجل <3", threadID);
   };
